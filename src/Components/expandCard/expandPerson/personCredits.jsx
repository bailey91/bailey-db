...
/* function Crew() {
      return (
        <React.Fragment>
          {crew.map((crew) => (
            <Accordion defaultActiveKey="1" key={crew.credit_id}>
              <Card>
                <ContextAwareToggle eventKey="0">
                ...
                <Accordion.Collapse eventKey="0">
                  <Card.Body>
                    ...
                    <Button
                      onClick={() =>
                        request.handleRequest(crew.id, crew.media_type)
                      }
                    >
                      View Details
                    </Button>
...
/*{this.state.roles.map((roles) => (
              <Accordion defaultActiveKey="1" key={roles.id}>
                <Card>
                  <Card.Header>
                    <Accordion.Toggle as={Button} variant="link" eventKey="0">
                      {roles.media_type === "movie"
                        ? roles.title +
                          " - " +
                          roles.character +
                          " - " +
                  ...
                  <Accordion.Collapse eventKey="0">
                    <Card.Body>
                      ...
                      <Button
                        onClick={() =>
                          this.props.request.handleRequest(
                            roles.id,
                            roles.media_type
                          )
                        }
...
/* {crew.map((crew) => (
              <Accordion defaultActiveKey="1" key={crew.id}>
                <Card>
                  <Card.Header>
                    <Accordion.Toggle as={Button} variant="link" eventKey="0">
                      {crew.media_type === "movie"
...
/*<DropdownButton id="dropdown-basic-button" title="Dropdown button">
          <Dropdown.Item>Sort By</Dropdown.Item>
          <Dropdown.Item>Date</Dropdown.Item>
          <Dropdown.Item>Vote Average</Dropdown.Item>
          <Dropdown.Item>Popularity</Dropdown.Item>
        </DropdownButton>*/

//{credits.map((credits) => ())}

// need to do a sort on these credits to sort them by most recent first instead of a random order
// Also consider slicing these to like 10 results and doing a click here for more?
// Also think of a way to have a link on this as adding anther element here is causing a crashes
